...
(function($) {
    var JFMFS = function(element, options) {
      ...
      excluded_friends_graph = arrayToObjectGraph(settings.exclude_friends),
        ...
        FB.api('/me/friends?fields=' + settings.friend_fields, function(response) {
            ...
            $.each(sortedFriendData, function(i, friend) {
              if(! (friend.id in excluded_friends_graph)) {
                ...
                var friendEl = "<div class='jfmfs-friend " + selectedClass + " ' id='" + friend.id +"'";
                for(k in friend) {
                  if(k != "id" && k != "name") {
                    friendEl += " data-" + k.replace(/_/g, '-') + "='" + friend[k] + "'";
                  }
                }
                friendEl +=  "><img/><div class='friend-name'>" + friend.name + "</div></div>";
                buffer.push(friendEl);
              }
            });
        ...
        this.getSelectedIdsAndData = function() {
            var selected = [];
            $.each(elem.find(".jfmfs-friend.selected"), function(i, friend) {
                var data = {id: $(friend).attr("id"), name: $(friend).find(".friend-name").text()};
                var elData = $(friend).data();
                for(k in elData) {
                  if(k != "inview") {
                    data[k] = elData[k];
                  }
